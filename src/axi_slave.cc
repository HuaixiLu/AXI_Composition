  ...
  // ------------------------------------------------------------------
{
  ...
  */
  ...
  wmodel.SetFetch( lConcat({s_axi_aresetn_w, s_axi_awvalid, s_axi_wvalid, write_ready, s_axi_bready}) );
  // Valid instruction: what means to have valid command (valid = 1)
  wmodel.SetValid( /*always true*/ BoolConst(true) );

  { // reset instruction
    auto instr = wmodel.NewInstr("Slave_W_Reset");
  ...
  { // Slave_AWW_Wait
    auto instr = wmodel.NewInstr("Slave_AWW_Wait"); 
    instr.SetDecode( (s_axi_awvalid == 0) & (s_axi_wvalid == 0) & ( s_axi_aresetn_w == 1 ) );
    // -- AW Channel -- //
  ...
  { // Slave_AW_Valid
    auto instr = wmodel.NewInstr("Slave_AW_Valid");
    instr.SetDecode( (s_axi_awvalid == 1) & (s_axi_wvalid == 0) & (s_axi_aresetn_w == 1));
    // -- AW Channel -- //
    // we're always ready for an address transaction 
    // if we're not doing something else (wactive = 0 & bwait = 0)
    // but when a transaction happens, awready will change to 0
    instr.SetUpdate(s_axi_awready, Ite(s_axi_awready == 1, BvConst(0,1), (tx_wactive == 0) & (tx_bwait == 0)));
    instr.SetUpdate(tx_wactive, Ite(s_axi_awready == 1, BvConst(1,1), tx_wactive));

    // update the info of the write transaction(burst_based transaction)
    instr.SetUpdate(tx_awlen,   Ite(s_axi_awready == 1, s_axi_awlen, tx_awlen));
    instr.SetUpdate(tx_awsize,  Ite(s_axi_awready == 1, s_axi_awsize, tx_awsize));
    instr.SetUpdate(tx_awaddr,  Ite(s_axi_awready == 1, s_axi_awaddr, tx_awaddr));
    instr.SetUpdate(tx_awburst, Ite(s_axi_awready == 1,  s_axi_awburst, tx_awburst));
  }

  { // Slave_W_Busy
    auto instr = wmodel.NewInstr("Slave_W_Valid"); 
    instr.SetDecode( (s_axi_wvalid == 1) & (s_axi_awvalid == 0) & ( s_axi_aresetn_w == 1));
    // tx_wactive ----- last_wr_beat : two important points
    instr.SetUpdate(s_axi_wready, Ite(s_axi_wready == 1, Ite(s_axi_wlast == 1, BvConst(0,1), write_ready), 
                                                         Ite(tx_wactive == 1, write_ready, s_axi_wready)));
    instr.SetUpdate(tx_wactive,  Ite(s_axi_wlast == 1 & s_axi_wready == 1, BvConst(0,1), tx_wactive));
    instr.SetUpdate(tx_bwait, Ite(s_axi_wlast == 1 & s_axi_wready == 1, BvConst(1,1), tx_bwait));
    instr.SetUpdate(s_axi_bvalid, Ite(s_axi_wlast == 1 & s_axi_wready == 1, BvConst(1,1), s_axi_bvalid));
    // ok resp
    instr.SetUpdate(s_axi_bresp, Ite(s_axi_wlast == 1 & s_axi_wready == 1, BvConst(0,2), s_axi_bresp));
    // info update
    instr.SetUpdate(tx_awlen, Ite(s_axi_wready == 1, tx_awlen - BvConst(1,8), tx_awlen));
    instr.SetUpdate(tx_awaddr, Ite(tx_awburst == BURST_INCR & s_axi_wready == 1, Concat(Extract(tx_awaddr,31,2) + BvConst(1,30) , BvConst(0,2)), tx_awaddr));
  }

    { // Slave_AWW_Valid
    auto instr = wmodel.NewInstr("Slave_AWW_Valid");
    instr.SetDecode( (s_axi_awvalid == 1) & (s_axi_wvalid == 1) & (s_axi_aresetn_w == 1));

    // s_axi_awready shouldn't equal to 1 in this case
    // update info between two channels
    instr.SetUpdate(tx_wactive, Ite(s_axi_awready == 1, BvConst(1,1), Ite(s_axi_wlast == 1 & s_axi_wready == 1, BvConst(0,1), tx_wactive)));
    instr.SetUpdate(tx_awlen,   Ite(s_axi_awready == 1, s_axi_awlen, Ite(s_axi_wready == 1, tx_awlen - BvConst(1,8), tx_awlen)));
    instr.SetUpdate(tx_awaddr,  Ite(s_axi_awready == 1, s_axi_awaddr, 
          Ite(tx_awburst == BURST_INCR & s_axi_wready == 1, Concat(Extract(tx_awaddr,31,2) + BvConst(1,30) , BvConst(0,2)), tx_awaddr)));
  ...
  { // B_Slave_Ready
    auto instr = wmodel.NewInstr("B_Slave_Ready");
    instr.SetDecode( (s_axi_bready == 1) & (s_axi_aresetn_w == 1) );
    instr.SetUpdate(s_axi_bvalid, Ite(s_axi_bvalid == 1, BvConst(0,1), BvConst(1,1)));
    instr.SetUpdate(tx_bwait,     Ite(s_axi_bvalid == 1, BvConst(0,1), BvConst(1,1)));
  }
